package Lecture.AScala.Afunctionalprogram

object CurringandPartial extends App{
  // curried functions
  ...
  println(numbers.map(seriousFormat)) // compiler does sweet eta-expansion for us
  println(numbers.map(preciseFormat)) // compiler does sweet eta-expansion for us
